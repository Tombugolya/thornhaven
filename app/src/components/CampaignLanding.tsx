...
function ThornhavenEmblem({ size = 120 }: { size?: number }) {
  const c = size / 2
  const r = size * 0.42
  return (
...
const campaignEmblems: Record<string, React.FC<{ size?: number }>> = {
...
export default function CampaignLanding({ onEnterCampaign, onSignOut }: CampaignLandingProps) {
  ...
  const handleEnter = (campaignId: string) => {
  ...
  return (
    <div className="min-h-screen bg-bg-deep flex flex-col items-center justify-center relative overflow-hidden">
      ...
      >
        ...
        <p className="text-text-muted text-sm tracking-widest uppercase mb-6">DM Companion</p>
        ...
        <div className="flex flex-wrap justify-center gap-6 w-full max-w-3xl">
          {campaigns.map((c) => (
            <CampaignCard key={c.id} campaign={c} onEnter={handleEnter} />
          ))}
        </div>
      </div>
    </div>
