  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4 flex-wrap">
        ...
        <CoreStat icon={Heart} label="HP" value={`${stats.hp}/${stats.maxHp}`} color="text-success-light" />
...
function CoreStat({ icon: Icon, label, value, sub, color }) {
  return (
...
export default function CharacterCard({ npc, onClose }) {
    ...
    <div className="bg-bg-surface border border-gold-dim/20 rounded-2xl overflow-hidden animate-in">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-parchment-dim italic mt-0.5">
            "{npc.vibe}"
          </p>
        </div>
        <button
            ...
            <div className="space-y-2">
              {npc.dialogueLines.map((d, i) => (
                <div key={i} className="bg-bg-base/50 rounded-lg p-3 border border-bg-elevated/30">
                  ...
                  <p className="text-sm text-parchment/90 italic leading-relaxed">
                    {d.line}
                  </p>
                </div>
              ))}
