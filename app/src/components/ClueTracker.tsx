...
import { Search, Check, MapPin, Lightbulb, ChevronDown, ChevronRight } from "lucide-react"
import { useCampaign } from "../hooks/useCampaign"
import { usePersistedState } from "../hooks/usePersistedState"
import ShowButton from "./ShowButton"
import type { Clue } from "../types/campaign"
...
const ClueCard = memo(function ClueCard({ clue, found, onToggle }: ClueCardProps) {
    ...
    >
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        >
          ...
          <div className="min-w-0">
            <h4
              className={`text-sm font-medium ${found ? "text-success-light" : "text-parchment"}`}
            >
              {clue.name}
            </h4>
        ...
        {clue.handout && (
          <ShowButton type="handout" id={clue.handout} label="Handout" buttonText="Handout" />
        )}
      ...
      {expanded && (
        <div className="px-4 pb-4 ml-9 space-y-3">
          <p className="text-sm text-parchment/80 leading-relaxed">{clue.description}</p>

          {clue.dc && (
            <div className="flex items-center gap-2 text-xs">
              <span className="text-text-muted">Check:</span>
              <span className="text-gold bg-gold/10 px-2 py-0.5 rounded">{clue.dc}</span>
            </div>
          )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {clue.leadsTo.map((lead, i) => (
                <span key={i} className="text-xs bg-info/10 text-info px-2 py-0.5 rounded-full">
                  {lead}
                </span>
              ))}
            </div>
...
export default function ClueTracker() {
  ...
  const [foundIds, setFoundIds] = usePersistedState<string[]>(`dm:${campaign.id}:clueState`, [])
  const [sessionFilter, setSessionFilter] = useState("all")

  const toggleClue = useCallback(
    (id: string) => {
      setFoundIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]))
    },
    [setFoundIds],
  )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-bg-surface/40 border border-warning/15 rounded-xl p-5">
        ...
        <p className="text-xs text-parchment-dim mb-3 italic">
          Every important clue is available in at least 3 ways. Use these failsafes in order:
        </p>
        <div className="space-y-3">
          {campaign.failsafes.map((f) => (
            <div key={f.order} className="flex items-start gap-3">
              ...
              <div>
                <div className="text-xs text-warning font-medium">{f.trigger}</div>
