...
const handwritingStyles = {
  shaky: { fontVariationSettings: "'wght' 500", letterSpacing: "0.02em", transform: "rotate(-0.5deg)" },
  ...
  crude: { fontVariationSettings: "'wght' 600", letterSpacing: "0.03em", textTransform: "uppercase" },
}

function Decoration({ type, paperBg }) {
  if (type === "torn-edge") {
    return (
      <div className="absolute top-0 left-0 right-0 h-4 pointer-events-none" style={{
        background: paperBg,
        clipPath: "polygon(0% 0%, 3% 60%, 7% 20%, 12% 70%, 18% 30%, 22% 80%, 28% 10%, 33% 60%, 38% 25%, 44% 75%, 50% 15%, 55% 65%, 60% 30%, 66% 70%, 72% 20%, 78% 80%, 83% 35%, 88% 65%, 93% 15%, 97% 55%, 100% 0%)",
  ...
  if (type === "ink-stain") {
    return (
      <div className="absolute bottom-16 right-8 w-32 h-24 pointer-events-none opacity-15" style={{
  ...
  if (type === "pin-holes") {
    return (
      <>
        <div className="absolute top-3 left-1/2 -translate-x-1/2 w-2.5 h-2.5 rounded-full pointer-events-none" style={{
          background: "rgba(0,0,0,0.15)",
          boxShadow: "inset 0 1px 2px rgba(0,0,0,0.3)",
        }} />
        <div className="absolute top-3 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full pointer-events-none" style={{
  ...
  if (type === "coffee-ring") {
    return (
      <div className="absolute bottom-12 right-12 w-20 h-20 rounded-full pointer-events-none opacity-15" style={{
        border: "3px solid #6b4226",
        borderRadius: "50%",
        background: "radial-gradient(circle, transparent 55%, rgba(107,66,38,0.1) 60%, transparent 70%)",
      }} />
    )
  }
  if (type === "crumpled") {
  ...
  if (type === "smudge") {
    return (
      <div className="absolute bottom-20 right-6 w-24 h-16 pointer-events-none opacity-10" style={{
...
function ContentBlock({ block, handwriting, paperColor }) {
  const baseStyle = {
  ...
  if (block.style === "annotation") {
    return (
      <p className="text-sm italic text-center opacity-50 my-2" style={{
        ...baseStyle,
...
export default function HandoutDisplay({ handout }) {
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="text-center mb-6">
            <p className="text-xs uppercase tracking-[0.3em] mb-2 opacity-40" style={{
              fontFamily: "var(--font-body)",
              color: paper.color,
            }}>
              {handout.paperType === "rock" ? "Scratched on Stone" : "Handout"}
            </p>
            <h2 className="text-sm font-medium tracking-wider opacity-50" style={{
              fontFamily: "var(--font-body)",
              color: paper.color,
            }}>
              {handout.title}
            </h2>
            <div className="w-16 h-px mx-auto mt-3" style={{
