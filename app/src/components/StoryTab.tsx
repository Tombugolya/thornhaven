...
import type { Section as SectionType } from "../types/campaign"
...
function Section({ section, forceState }: SectionProps) {
    ...
    >
      <div
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 text-left cursor-pointer transition-colors duration-150 ${
          open ? "bg-bg-surface/80" : "bg-bg-surface/40 hover:bg-bg-surface/60"
        }`}
      >
        {open ? (
        ...
        <span className="font-medium text-parchment text-sm">{section.title}</span>
        <span className="ml-auto flex items-center gap-2">
          {section.handout &&
            (Array.isArray(section.handout) ? (
...
export default function StoryTab() {
  ...
  const [activeSession, setActiveSession] = usePersistedState(`dm:${campaign.id}:activeSession`, 1)

  const session = campaign.sessions.find((s) => s.id === activeSession)
  const [forceState, setForceState] = useState<ForceState | null>(null)
